...
angular
  ...
  .config([
    ...
    function ($stateProvider, $urlRouterProvider) {
      $stateProvider
                ...
                return $http
                  ...
                  .then(function (data, status) {
                    ...
                    for (var i = 0; i < items.length; i++) {
                      ret[items[i]._id] = transformTextToHtml(
                        $sce,
                        items[i].__text,
                        parseHtml
                      );
                    ...
                    ret.fillInRequiredFields =
                      ret.fillInRequiredFields ||
                      $sce.trustAsHtml("Please fill in all required fields.");
        ...
        .state("root.step4", {
          // comments for best and worst rating
          url: "/step4",
  ...
  .controller("RootCtrl", [
    ...
    function (
      ...
      for (var i = 0; i < items.length; i++) {
        if (
          items[i].attributes &&
          items[i].attributes.getNamedItem("id") &&
          items[i].attributes.getNamedItem("id").value === "form"
        ) {
          form = items[i];
          break;
        }
        ...
        for (var i = 0; i < formElements.length; i++) {
          ...
          if (formElements[i].nodeName === "label") {
            ...
            el.scale = formElements[i].attributes.getNamedItem("scale")
              ...
              : [];
            el.required =
              formElements[i].attributes.getNamedItem("required") &&
              formElements[i].attributes
                .getNamedItem("required")
                .value.toLowerCase() === "true";
            el.maxLength =
              formElements[i].attributes.getNamedItem("maxlength") &&
              formElements[i].attributes.getNamedItem("maxlength").value;
            el.restricted =
              formElements[i].attributes.getNamedItem("restricted") &&
              formElements[i].attributes.getNamedItem("restricted").value &&
              formElements[i].attributes.getNamedItem("restricted").value
                .length > 0;
            if (
              formElements[i].firstChild &&
              formElements[i].firstChild.nodeValue
            ) {
              el.options = formElements[i].firstChild.nodeValue.split(";");
            }
            if (
              el.inputType === "checkbox" ||
              el.inputType.indexOf("rating") === 0
            ) {
          ...
          el.bg =
            formElements[i].attributes.getNamedItem("bg") &&
            formElements[i].attributes.getNamedItem("bg").value === "true";
      ...
      $scope.progress = function () {
        ...
        var allStatements = inGrid.concat(
          $scope.sortedStatements.agree,
          $scope.sortedStatements.neutral,
          $scope.sortedStatements.disagree
        );
        ...
        if (config.showStep4 === "true") {
          ...
          for (
            var i = 0;
            i <
            $scope.sortedStatements.grid[
              $scope.sortedStatements.grid.length - 1
            ].length;
            i++
          ) {
            var statement =
          ...
          for (var i = 0; i < $scope.survey.formElements.length; i++) {
            if ($scope.survey.formElements[i].required) {
              if ($scope.survey.formElements[i].inputType === "checkbox") {
                ...
                $scope.survey.formElements[i].value &&
                $scope.survey.formElements[i].value.length
              ) {
                current++;
              }
              total++;
  ...
  .directive("draggableStatementFirst", [
    function () {
      return {
        restrict: "A",
        scope: {
    ...
    function ($compile, $log) {
      return {
        ...
        link: function (scope, element, attrs) {
          ...
          // initial statement
          if (
            scope.dynamicallyCreateInitialStatement === "true" &&
            scope.cell.statement
          ) {
  ...
  .controller("Step1Ctrl", [
    ...
    function (
      ...
      $scope.allCards = function () {
        return (
          SortedStatements.list.length +
          SortedStatements.agree.length +
          SortedStatements.neutral.length +
          SortedStatements.disagree.length +
          getOrderedStatementsFromGrid(SortedStatements).length
        );
      };
  ...
  .controller("Step4Ctrl", [
    ...
    function (
      ...
      $scope.agreeColumn =
        SortedStatements.grid[SortedStatements.grid.length - 1];
    },
  ])
      ...
      function makeRequiredEmptySurveyFieldsRed() {
        for (var i = 0; i < Survey.formElements.length; i++) {
          ...
          if (formElement.required) {
            if (
              formElement.inputType === "text" ||
              formElement.inputType === "textarea"
            ) {
              formElement.red =
                !formElement.value || formElement.value.length === 0;
    ...
    function (
      ...
      function makeSortString(sortedStatements) {
        ...
        for (var i = 0; i < sortedStatements.length; i++) {
          var rating = getRatingForStatement(
            map,
            sortedStatements[i],
            SortedStatements
          );
      ...
      function makeParamsObject() {
        ...
        for (var i = 0; i < SortedStatements.grid[0].length; i++) {
          ...
          ret["comment" + statement._id] =
            "(s" + statement._id + ") " + statement.comment;
        }

        for (
          var i = 0;
          i < SortedStatements.grid[SortedStatements.grid.length - 1].length;
          i++
        ) {
          var statement =
            SortedStatements.grid[SortedStatements.grid.length - 1][i]
              .statement;
          ret["comment" + statement._id] =
            "(s" + statement._id + ") " + statement.comment;
        }
        ...
        for (var i = 0; i < Survey.formElements.length; i++) {
          if (Survey.formElements[i].type === "input") {
            ret["form" + ct] = Survey.formElementToString(
              Survey.formElements[i]
            );
            ct++;
          }
        }
      ...
      $scope.mailSubject =
        "HtmlQ/" + config["studyTitle"] + " " + config["_version"];
  ...
  .controller("PrintCtrl", [
    ...
    function (
      ...
      $scope.date = new Date();
      $scope.totalDuration = Math.round(
        Duration[0] + Duration[1] + Duration[2] + Duration[3] + Duration[4]
      );
  ...
  .controller("ModalInstanceCtrl", [
    ...
    "ok",
    "textAlign",
    function ($scope, $modalInstance, messageHead, message, ok, textAlign) {
  ...
  .run([
    ...
    "$log",
    "$state",
    function ($rootScope, $modalStack, $log, $state) {
      $rootScope.$on(
        "$stateChangeError",
